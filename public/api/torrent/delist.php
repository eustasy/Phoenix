...
// appears on the public index. POST only. Authorization: an `Authorization:
// Bearer <key>` header — an owner key (its own torrents) or the '*' admin (any
// torrent) — or an admin.php session carrying a CSRF token. Parameters come
// from the POST body. Responds with JSON by default and XML with ?xml.
