...
;(function (global) {
  ...
  function parseTorrent(arrayBuffer) {
    ...
    function parseString() {
      const len = parseInt(readDigits(58), 10)
    ...
    function parseValue() {
      ...
      if (b === 105) {
        pos++
        return parseInt(readDigits(101), 10)
      }
      if (b === 108) {
        pos++
        const arr = []
        while (bytes[pos] !== 101) arr.push(parseValue())
        pos++
        return arr
      }
      if (b === 100) {
  ...
  async function torrentInfo(arrayBuffer) {
    ...
    const trackers = []
    const addTracker = (u) => {
      const s = u instanceof Uint8Array ? decode(u) : ""
    ...
    if (Array.isArray(torrent["announce-list"]))
      for (const tier of torrent["announce-list"]) if (Array.isArray(tier)) for (const u of tier) addTracker(u)

    const webseeds = []
    const addSeed = (u) => {
      const s = u instanceof Uint8Array ? decode(u) : ""
      if (s && !webseeds.includes(s)) webseeds.push(s)
    }
