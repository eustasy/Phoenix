...
// form (first $short hex chars) is shown; the full hash rides on data-copy so
// phCopy() (assets/copy.js, which wires every .ph-copy) can copy it.
...
function view_hash_html(string $hash, int $short = 12): string
{
    ...
    return '<span class="hash">'.
        ...
        '<button class="ph-copy" type="button" title="Copy info hash" aria-label="Copy info hash" data-copy="'.$full.'">'.
