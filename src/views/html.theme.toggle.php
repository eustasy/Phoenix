...
// ph-theme-fixed on auth pages); $style passes an inline style string.

function view_theme_toggle_html(string $light = 'Light', string $dark = 'Dark', string $extra_class = '', string $style = ''): string
{
    $class = 'ph-theme-toggle'.($extra_class !== '' ? ' '.$extra_class : '');
    $style_attr = $style !== '' ? ' style="'.htmlspecialchars($style, ENT_QUOTES, 'UTF-8').'"' : '';

    return '<button class="'.$class.'" type="button" onclick="phToggleTheme()"'.$style_attr.'>
