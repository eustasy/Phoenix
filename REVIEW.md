...
### 1.1 Empty `admin_password` ⇒ the admin panel is unauthenticated  *(confirmed)*
...
### 1.2 The SQL layer has no defense-in-depth  *(confirmed — architectural)*
There are **zero** `mysqli_real_escape_string` calls and no prepared statements;
every query is string-concatenated. It's safe *today* only because every interpolated
...
### 1.3 No brute-force throttle on admin login  *(confirmed)*
...
### 1.4 All runtime errors are silenced  *(confirmed — operability/security)*
...
### 1.5 `Access-Control-Allow-Origin: *` on every response  *(confirmed — low)*
...
### 1.6 Client-controlled source IPs — defaults are safe; reinforce  *(confirmed — low)*
...
- **Stale/contradictory comment:** `public/admin.php` says *"This page is not secure.
  It should not be deployed in a production environment."* despite real auth existing —
  clarify or remove so operators aren't misled. *(confirmed)*
