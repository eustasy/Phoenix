...
class DbDropTableTest extends PhoenixTestCase {
	...
	public function testDropsExistingTable(): void {
		...
		$this->assertTrue(db_drop_table(self::$connection, self::$settings, '__TEST__'));
	}

	public function testDropMissingTableIsNoOp(): void {
		// IF EXISTS in the SQL means dropping a missing table is not an error.
		$this->assertTrue(db_drop_table(self::$connection, self::$settings, '__TEST_NOT_THERE__'));
	}
	...
	public function testReturnsFalseAndEchoesErrorOnSqlFailure(): void {
		...
		try {
			...
			ob_start();
			$result = db_drop_table(self::$connection, self::$settings, 'bad`name');
			$output = ob_get_clean();
