...
1. The `_cron/hourly/backup-database.php` script reads its database credentials from `_settings/phoenix.custom.php`, so it needs no editing — it only requires the `mysqldump` binary to be available on the server. Optionally set `$settings['backup_dir']` to an absolute path **outside the web root** (the default, `_backups/` in the project root, is only safe once you deny `_` prefixed paths — see [Securing the Web Root](#securing-the-web-root)) and `$settings['backup_rotate']` (days of backups to keep; default 30).
