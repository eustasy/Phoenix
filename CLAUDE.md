...
- **`src/functions/`** — one function per file. File `peer.new.php` defines `peer_new()`. Functions are pure-ish PHP (no top-level execution beyond defining the function). Business logic helpers: sanitization, validation, address parsing, peer selection strategies, etc. One function per file, `////    <function_name>` header comment, then a brief description, then the function.
