...
- ~~No `password_needs_rehash()` on the admin password~~ — FIXED (`e4cfd1f`, see Remediation log).
- ~~Stale comments calling `maybe_binary_to_hex()` "the project's SQL-injection defense"~~ —
  FIXED. The real defense is parameterization (`mysqli_execute_query` with bound params); the
  sanitizer is input *validation*. Corrected across the affected controllers, the sanitizer's
  own header, `docs/database.md`, and `CLAUDE.md` so a future change doesn't "trust the
  sanitizer" and revert a model to string concatenation for a non-hex field.
